...
GatsbyJs server will start in `localhost:3010`
...
NextJs Server will start in `localhost:3030`
...
Now if you start your `gatsbyjs` server with `yarn gatsby-dev` then you will get your server running on `localhost:3010`
...
To start the server for `nextjs` you have to run `yarn nextjs-dev` and the server will start on `locahost:3030`.
...
docker run -it -p 3000:3030 superprops/landing-next:latest
...
docker run -it -p 3010:3010 superprops/landing-gatsby:latest
